...
import numpy as np
from utils import puasson, plot_pn1n2, g2, ts, SIZE
...
def get_g2(rho, shots=100, name="puasson"):
    bs = BeamSplitter(rho)
    pn1n2 = get_pn1n2_from_4kron(bs.get_evolved_rho(), bs.num_basis)
    plot_pn1n2(pn1n2, name)
    ls = np.random.choice(np.arange(bs.num_basis**2), size=SIZE*shots, p=pn1n2.reshape(-1)/np.sum(pn1n2))
    return g2(ls, bs) 

if __name__ == "__main__":
    n = 40
    rho_ps = np.diag([i for i in puasson(1)(n)])

    coh_in_g, coh_in_d = get_g2(rho_ps)
    print("puasson -- g2 = ", coh_in_g, "+-" ,coh_in_d)

    n = 40
    rho_ts = np.diag([i for i in ts(0.5)(n)])
